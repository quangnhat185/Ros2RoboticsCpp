...
public:
  ...
  EKFSlam(float max_range, float m_dist_th, int state_size, int lm_size,
          MatrixXd &rf_id, MatrixXd &u)
      : MAX_RANGE(max_range), M_DIST_TH(m_dist_th), STATE_SIZE(state_size),
  ...
  vector<MatrixXd> ekf_slam(MatrixXd &xEst, MatrixXd &PEst, MatrixXd &ud,
                            MatrixXd &z) {
    // Predict
    int S = STATE_SIZE;
